...
//Global variable
static __attribute((aligned(4))) BOOL bPWMIRQFlag=FALSE;  //IRQ enable flag, set after PWM IRQ enable
static __attribute((aligned(4))) BOOL bPWMTimerOpenStatus[PWM_TIMER_NUM]; //timer flag which set after open(for disable IRQ decision)
static __attribute((aligned(4))) BOOL bPWMTimerStartStatus[PWM_TIMER_NUM]; //timer flag which set after Start count(to avoid incorrectly stop procedure)
static __attribute((aligned(4))) BOOL bPWMTimerMode[PWM_TIMER_NUM]; //PWM timer toggle/one shot mode
static __attribute((aligned(4))) BOOL volatile bPWMIntFlag[PWM_TIMER_NUM]; //interrupt flag which set by ISR
...
#define PWM_IRQn    1
#define ENABLE_IRQ  1
void sysInstallISR(INT irq_level, INT pwm_irq, PVOID pwm_isr){}
void sysSetLocalInterrupt(INT irq_en){}
void sysEnableInterrupt(INT pwm_irq){}
void sysDisableInterrupt(INT pwm_irq){}
//--------------------
...
INT PWM_Timer(INT timer_num)
{
    ...
    else if(timer_num == PWM_TIMER1)
    {
        ...
        pwmIoctl(timer_num, ENABLE_PWMGPIOOUTPUT, PWM_TIMER1, PWM1_GPB3);
    }
    ...
    pwmvalue.field.cnr=5999;
    pwmvalue.field.cmr=2999;
